...
new_int_0x70:
  ...
  .w0:                                    
      ...
      jnz .w0                            ;以上代码对于更新周期结束中断来说 
                                         ;是不必要的 
      xor al,al
      or al,0x80						;阻断NMI 和访问0x00偏移地址，读出当前时间（秒）
      out 0x70,al
      ...
      iret					;iret 是Interrupt Return，即中断返回，从栈中弹出cs、ip和标志寄存器，返回被打断的地方继续执行程序
...
;-------------------------------------------------------------------------------
start: 
      mov ax,[stack_segment]			;令段寄存器ds和栈寄存器ss指向用户自己的寄存器段和栈段
      mov ss,ax							;栈段
      mov sp,ss_pointer
      mov ax,[data_segment]				;数据段
      mov ds,ax
      ...
      cli                                ;防止改动期间发生新的0x70号中断  这条指令将标志寄存器的中断标志IF清0以禁止处理器响应中断，即：关中断
